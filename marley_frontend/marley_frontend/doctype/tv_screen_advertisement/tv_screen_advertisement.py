...
class TVScreenAdvertisement(Document):
	def validate(self):
		...
		overlap = frappe.db.get_all('TV Screen Advertisement', filters= {"name": ["!=", self.name], "enabled": True}, or_filters=[['from_date', 'between', [getdate(self.from_date), getdate(self.to_date)]], ['to_date', 'between', [getdate(self.from_date), getdate(self.to_date)]]], pluck='file_name')
		if overlap:
